...
read_compliance <- function(){
  compliance <- read.csv("Italy/data/compliance.csv", stringsAsFactors = FALSE)
  print(compliance$date)
  compliance$date = as.Date(compliance$date, format = '%Y-%m-%d')
  return(compliance)
}
...
read_google_mobility <- function(Country){
  ...
  google_mobility[, c(8,9,10,11,12,13)] <- google_mobility[, c(8,9,10,11,12,13)]/100
  google_mobility[, c(8,9,10,11,12)] <- google_mobility[, c(8,9,10,11,12)] * -1
  google_mobility<-google_mobility[,c(2,3,7,8,9,10,11,12,13)]
