...
process_covariates <- function(regions, mobility, compliance, intervention, d, ifr.by.country, N2, formula, formula_partial){
  ...
  # compliance_length <- length(colnames(compliance))
  
  covariate_list <- list()
  covariate_list_partial <- list()
  k=1
  
  compliance$date <- ymd(compliance$date)
  compliance <- na.locf(compliance)
  compliance <- compliance[order(compliance$date),]  # ensure date ordering
  ...
  for (Country in regions){
    ...
    # *************************************************************************************
    print(paste(Country,'CREATING compliance data...'))
    compliance1 <- data.frame("country" = rep(Country, length(compliance$date)),
                              "date" = compliance$date,
                              "compliance1" = compliance$compliance1,
                              "compliance2" = compliance$compliance2,
                              "compliance3" = compliance$compliance3,
                              stringsAsFactors=F)
    compliance_length <- length(colnames(compliance1))
    # Padding in compliance data for dates before first time data exists
    if (compliance1$date[1] > date_min){
      print(paste(Country,'In padding compliance backwards'))
      pad_days <- compliance1$date[1] - date_min
      pad_dates <- date_min + days(1:pad_days[[1]]-1)
      PAD = rep(0,pad_days)
      padded_data <- data.frame("country" = rep(Country, pad_days),
                                "date" = pad_dates,
                                ...
                                "compliance3" = PAD,
                                stringsAsFactors=F)
      compliance1 <- bind_rows(padded_data, compliance1)
      compliance1 = compliance1[order(compliance1$date),]  # ensure date ordering
      }
    ...
    mobility1 = mobility1[order(mobility1$date),]  # ensure date ordering
    mobility1<-na.locf(mobility1)
    print(paste(Country,'In padding mobility forward OK!'))
    
    # ******************************************************************************
    # forecasting compliance data
    date_max = max(d1$DateRep)
    # print(date_max)
    date_compliance_max = max(compliance1$date)
    # print(date_compliance_max)
    
    if (date_compliance_max < date_max){
      print(paste(Country,'In padding compliance forward'))
      forecast_days <- date_max - date_compliance_max
      # print(forecast_days)
      forecast_dates <- date_compliance_max + days(1:forecast_days[[1]])
      # print(forecast_dates)
      forecast_data <- data.frame("country"=rep(Country,forecast_days[[1]]),"date" = as.Date(forecast_dates,format="%Y-%m-%d"))
      # print(forecast_data)
      fore<-list()
      
      for(i in 3:compliance_length){
        com <- compliance1 %>% select(date,colnames(compliance1[i]))
        
        # impute last week
        #fore[[i]] <- c(0:(as.numeric(forecast_days)-1)) %>% map(function(s) ts[length(ts)-6+mod(s,7)])
        f <- c(0:(as.numeric(forecast_days)-1)) %>% map(function(s) com[nrow(com)-6+mod(s,7),]) %>% bind_rows()
        fore[[i]]<-f[,2]
        # print(fore[[i]])
      }
      
      print(paste(Country,'PADDING compliance data...'))
      padded_data <- data.frame("country" = rep(Country, forecast_days[[1]]),
                                "date" = forecast_dates,
                                "compliance1" = fore[[3]],
                                "compliance2" = fore[[4]],
                                "compliance3" = fore[[5]],
                                stringsAsFactors=F)
      compliance1 <- bind_rows(compliance1,padded_data)
    }
    print(paste(Country,'Ordering compliance forward...'))
    compliance1 = compliance1[order(compliance1$date),]  # ensure date ordering
    compliance1<-na.locf(compliance1)
    # *************************************************************************************
    print(paste(Country,'In padding compliance forward OK!'))
    ...
    compliance1 = compliance1[index2:nrow(compliance1),]
    ...
    compliance1[N:(N+forecast),] <- compliance1[N,]
    ...
    df_features <- data.frame('schools_universities' = intervention_state$schools_universities, 
                              ...
                              'compliance1' = compliance1$compliance1,
                              'compliance2' = compliance1$compliance2,
                              'compliance3' = compliance1$compliance3,
  ...
  dm=dim(stan_data$X)
  for(j in 1:3){ # for covariates ORIG dm[3]
    for(i in 1:dm[1]){ # regions
      raw=stan_data$X[i,,j]
      if(all(raw!=0)){
        # print(paste("X All row != 0",j,i))
        top = raw[raw>=0]
        bottom = raw[raw<=0]
        adjusted=raw
  ...
  for(j in 1:3){ # for covariates ORIG dm[3]
    for(i in 1:dm[1]){ # regions
      ...
      if(all(raw!=0)){
        # print(paste("X_partial All row != 0",j,i))
        top = raw[raw>=0]
        bottom = raw[raw<=0]
        adjusted=raw
        if(sum(top==1)!=length(top)) top=rescale(top,to=c(0,1));adjusted[raw>=0]=top
