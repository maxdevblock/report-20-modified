...
Sys.setenv(FULL = "FALSE") # to run  in full mode
...
if(length(args) == 0) {
  args = c('base-italy', 'google', 'interventions',
           '~ -1 + transit + residential + averageMobility + compliance1 + compliance2 + compliance3',
           '~ -1 + transit + residential + averageMobility + compliance1 + compliance2 + compliance3'
  ) 
} 
...
processed_data <- process_covariates(regions = regions, mobility = mobility, compliance = compliance, intervention = interventions, 
